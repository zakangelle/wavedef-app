...
export default class ThreeOscSynth
{
  constructor()
  {
    ...
    let { synth, midi, oscGroup, filter, mixer, vca, envelope, lfo, delay, eq3, computerKeyboard, frequencyAnalyzer, ringmod, chorus, panner } = this;
    ...
    synth.connect(mixer).to(vca).to(filter).to(ringmod).to(delay).to(eq3).to(chorus).to(panner).to(frequencyAnalyzer).output();
